 ...
 */
void tablahash_insertar(TablaHash* tabla, void* clave, void* dato, FuncionIgualdad igual) {
  ...
  if (igual(tabla->tabla[idx].clave, NULL))
    tabla->numElems++;
  else
    {      
      while(!(igual(tabla->tabla[idx].clave, NULL)))
      {
        idx++;
        idx = idx % tabla->capacidad;
      }
 ...
 */
void* tablahash_buscar(TablaHash* tabla, void* clave, FuncionIgualdad igual) {
  // Calculamos la posición de la clave dada, de acuerdo a la función hash.
  unsigned idx = tabla->hash(clave);
  idx = idx % tabla->capacidad;
